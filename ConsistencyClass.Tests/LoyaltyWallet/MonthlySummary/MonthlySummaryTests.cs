...
public class MonthlySummaryTests
{
    ...
    private LoyaltyWalletEvent Earned(int points, DateTime at) =>
        new LoyaltyPointsEarned(_walletNumber, _owner, LoyaltyPoints.Of(points), at);

    private LoyaltyWalletEvent Redeemed(int points, DateTime at, int burned) =>
        new LoyaltyPointsRedeemed(_walletNumber, _owner, _owner, LoyaltyPoints.Of(points), LoyaltyPoints.Of(burned), at);

    private LoyaltyWalletEvent Reset(DateTime at) =>
        new RedemptionWindowReset(_walletNumber, _owner, at);

    private static MonthlySummary Fold(params LoyaltyWalletEvent[] events) =>
    ...
    {
        var summary = Fold(Earned(100, _june), Redeemed(40, _june, 38), Redeemed(10, _june, 10), Reset(_june));
